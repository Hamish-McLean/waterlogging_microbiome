...
# Add new column to colData to label treatments
FUN$colData$treat_label <- ifelse(FUN$colData$treatment == 0, "Control", "Waterlogged")
BAC$colData$treat_label <- ifelse(BAC$colData$treatment == 0, "Control", "Waterlogged")

# Add new column to colData for experiment year
FUN$colData$exp_year <- ifelse(FUN$colData$experiment == 1, "2022", "2023")
BAC$colData$exp_year <- ifelse(BAC$colData$experiment == 1, "2022", "2023")

# Add new column to colData for timepoint => treatment rep
FUN$colData$treat_rep <- ifelse(FUN$colData$timepoint == 1, "Single", "Repeated")
BAC$colData$treat_rep <- ifelse(BAC$colData$timepoint == 1, "Single", "Repeated")
FUN$colData$treat_rep <- factor(FUN$colData$treat_rep, levels = c("Single", "Repeated"))
BAC$colData$treat_rep <- factor(BAC$colData$treat_rep, levels = c("Single", "Repeated"))
...
FUN  <- create_dds(FUN, "FUN", "all")
BAC  <- create_dds(BAC, "BAC", "all")
...
print_asv_summary(EXP$BAC, "Bacteria Experiment ASV Summary") |> printTable()
...
top_asvs(counts(FUN$dds, normalize = TRUE), FUN$taxData, n = 10)
