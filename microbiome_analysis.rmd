...
exp_cn <- apply_to_subsets(EXP, copyNumberModelLMM, E_DESIGN) |> combine_tables()

time_cn <- apply_to_subsets(TIME, copyNumberModelLMM, T_DESIGN) |> combine_tables()
...
# combineAlphaPlot <- function(plots) {
#   unlist(plots, FALSE)
# }
...
exp_bc  <- apply_to_subsets(EXP, adonisModel, e_design) |> combine_tables()
...
apply_to_subsets(EXP, pcaLMM, E_DESIGN) |> combine_tables() |> 
  filter(P <= 0.05 & factor != "(Intercept)") |> printTable()

# EXP PCA linear mixed model
apply_to_subsets(TIME, pcaLMM, T_DESIGN) |> combine_tables() |> 
  filter(P <= 0.05 & factor != "(Intercept)") |> printTable()
