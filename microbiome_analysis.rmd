...
copyNumberPlot <- function(data, x, xlab = NULL) {
  ggboxplot(
    data,
    x = x,
    y = "log10_copy_number",
...
# Experiment
exp_cn_plot <- copyNumberPlot(filter(copy_number_combined, timepoint == 1), "exp_year", "Experimental replicate")
ggsave(filename = "CN_EXP.png", path = FIGURES_DIR)

# Timepoint
time_cn_plot <- copyNumberPlot(filter(copy_number_combined, experiment == 1), "treat_rep", "Waterlogging repetition")
ggsave(filename = "CN_TIME.png", path = FIGURES_DIR)

# Combined
collect_legends(exp_cn_plot | time_cn_plot, position = "bottom") + plot_annotation(tag_levels = "A")
ggsave(
  filename = "CN_combined.png", path = FIGURES_DIR,
...
time_alpha <- list(
  FUN = list(
    ...
    soil = bind_rows(
      singleAlphaLM(TIME$FUN$soil$alphaData, "FUN", "soil", "S.chao1", T_DESIGN),
      singleAlphaLM(TIME$FUN$soil$alphaData, "FUN", "soil", "shannon", T_DESIGN),
      singleAlphaLM(TIME$FUN$soil$alphaData, "FUN", "soil", "simpson", T_DESIGN)
    )
  ),
  BAC = list(
    root = bind_rows(
    ...
    soil = bind_rows(
      singleAlphaLM(TIME$BAC$soil$alphaData, "BAC", "soil", "S.chao1", T_DESIGN),
      singleAlphaLM(TIME$BAC$soil$alphaData, "BAC", "soil", "shannon", T_DESIGN),
      singleAlphaLM(TIME$BAC$soil$alphaData, "BAC", "soil", "simpson", T_DESIGN)
    )
  )
)
...
plotAlpha <- function(data, k, s, measures, design, xlab = NULL) {
  plot_alpha(
    data$countData, data$colData,
    design = design, colour = "treat_label",
    measures = measures, cbPalette = TRUE,
    type = "box"
  ) + #scale_colour_manual(values = cbPalette) +
    # theme(axis.title.x = element_blank()) +
    ggtitle(paste0(ifelse(k == "FUN", "Fungi", "Bacteria"), ": ", s)) + 
    labs(
      x = ifelse(!is.null(xlab), xlab, design),
      y = "α-diversity measure",
      color = NULL
    ) + 
...
exp_alpha_plots <- apply_to_subsets(EXP, plotAlpha, measures, "exp_year", "Experimental replicate") |> unlist(FALSE)

# ggarrange(exp_alpha_plots, ncol = 2, common.legend = TRUE, legend = "bottom")

exp_alpha_plot <- shared_legend(
  exp_alpha_plots[[1]], exp_alpha_plots[[2]], 
  exp_alpha_plots[[3]], exp_alpha_plots[[4]],
  ncol = 2, position = "right"
) |> plot_grid()

# exp_alpha_plot <- collect_legends(
#   (exp_alpha_plots[[1]] + exp_alpha_plots[[2]]) / 
#   (exp_alpha_plots[[3]] + exp_alpha_plots[[4]]),
#   position = "bottom"
# )
...
time_alpha_plots <- apply_to_subsets(TIME, plotAlpha, measures, "treat_rep", "Waterlogging repetition") |> unlist(FALSE)

time_alpha_plot <- shared_legend(
  time_alpha_plots[[1]], time_alpha_plots[[2]], 
  time_alpha_plots[[3]], time_alpha_plots[[4]],
  ncol = 2, position = "right"
) |> plot_grid()

# time_alpha_plot <- collect_legends(
#   (time_alpha_plots[[1]] + time_alpha_plots[[2]]) /
#   (time_alpha_plots[[3]] + time_alpha_plots[[4]]),
#   position = "bottom"
# )
...
# Combined
exp_alpha_plot / plot_spacer() / time_alpha_plot + 
  plot_annotation(tag_levels = "A") +
  plot_layout(heights = c(1, 0.05, 1))
# collect_axes(exp_alpha_plot / time_alpha_plot) + plot_annotation(tag_levels = "A")
# collect_legends(exp_alpha_plot / time_alpha_plot, position = "right") + plot_annotation(tag_levels = "A")

ggsave(
  filename = "alpha_combined.png", path = FIGURES_DIR,
...
cnAlphaPlot <- function(data, x, y, xlab = NULL, ylab = NULL) {
  ggboxplot(
    data,
    x = x,
    y = y,
...
exp_cn_alpha <- bind_rows(
  bind_rows(EXP$FUN$soil$alphaData, EXP$FUN$root$alphaData)[, type := "Fungi"],
  bind_rows(EXP$BAC$soil$alphaData, EXP$BAC$root$alphaData)[, type := "Bacteria"]
)

time_cn_alpha <- bind_rows(
  bind_rows(TIME$FUN$soil$alphaData, TIME$FUN$root$alphaData)[, type := "Fungi"],
  bind_rows(TIME$BAC$soil$alphaData, TIME$BAC$root$alphaData)[, type := "Bacteria"]
)

exp_cn_alpha_plot <- ggarrange(
  cnAlphaPlot(exp_cn_alpha, "exp_year", "log10_copy_number", "Experimental replicate", "log10 copy number"), #bquote(log[10] ~ "copy number")),
  cnAlphaPlot(exp_cn_alpha, "exp_year", "S.chao1", "Experimental replicate", "Chao1"),
  cnAlphaPlot(exp_cn_alpha, "exp_year", "shannon", "Experimental replicate", "Shannon"),
  cnAlphaPlot(exp_cn_alpha, "exp_year", "simpson", "Experimental replicate", "Simpson"),
  common.legend = TRUE,
  legend = "bottom"
)

time_cn_alpha_plot <- ggarrange(
  cnAlphaPlot(time_cn_alpha, "treat_rep", "log10_copy_number", "Waterlogging repetition", "log10 copy number"), #bquote(log[10] ~ "copy number")),
  cnAlphaPlot(time_cn_alpha, "treat_rep", "S.chao1", "Waterlogging repetition", "Chao1"),
  cnAlphaPlot(time_cn_alpha, "treat_rep", "shannon", "Waterlogging repetition", "Shannon"),
  cnAlphaPlot(time_cn_alpha, "treat_rep", "simpson", "Waterlogging repetition", "Simpson"),
  common.legend = TRUE,
  legend = "bottom"
)

exp_cn_alpha_plot / time_cn_alpha_plot + plot_annotation(tag_levels = "A")

ggsave(
  filename = "cn_alpha_combined.png", path = FIGURES_DIR,
...
exp_nmds_plots <- list(
  nmdsPlot(EXP$FUN$root$dds, shape = "exp_year"),
  nmdsPlot(EXP$FUN$soil$dds, shape = "exp_year"),
  nmdsPlot(EXP$BAC$root$dds, shape = "exp_year"),
  nmdsPlot(EXP$BAC$soil$dds, shape = "exp_year")
)

nmds_exp_combined <- ggarrange(
  plotlist = exp_nmds_plots,
  common.legend = TRUE,
  legend = "bottom",
  labels = c("Fungi: roots", "Fungi: soil", "Bacteria: roots", "Bacteria: soil"),
  font.label = list(size = 12, vjust = 1.5)
)

# nmds_exp_combined
ggsave("NMDS_exp.png", path = FIGURES_DIR)

# Timepoint

time_nmds_plots <- list(
  nmdsPlot(TIME$FUN$root$dds, shape = "treat_rep"),
  nmdsPlot(TIME$FUN$soil$dds, shape = "treat_rep"),
  nmdsPlot(TIME$BAC$root$dds, shape = "treat_rep"),
  nmdsPlot(TIME$BAC$soil$dds, shape = "treat_rep")
)

nmds_time_combined <- ggarrange(
  plotlist = time_nmds_plots,
  common.legend = TRUE,
  legend = "bottom",
  labels = c("Fungi: roots", "Fungi: soil", "Bacteria: roots", "Bacteria: soil"),
  font.label = list(size = 12, vjust = 1.5)
...
# Combined
collect_legends(nmds_exp_combined / plot_spacer() / nmds_time_combined) + 
  plot_annotation(tag_levels = "A") +
  plot_layout(heights = c(1, 0.01, 1))

ggsave(
  "NMDS_combined.png", path = FIGURES_DIR,
...
diffAbundanceGenus <- function(data, k, s, sample_min = 4, tax_conf = 0.5) {
  ...
  # Rename
  rownames(countData) <- gsub("_sensu_stricto", "", rownames(countData))
  taxData$genus <- gsub("_sensu_stricto", "", taxData$genus)
...
fullTaxa <- function(taxData, data) {
  cols <- c("kingdom", "phylum", "class", "order", "family", "genus")
  genera <- c(data$root$ASV, data$soil$ASV) |> unique()
  as.data.table(taxData)[genus %in% genera, ..cols] |> unique()
}


bind_rows(
  fullTaxa(FUN$taxData, exp_genus_da$FUN),
  fullTaxa(FUN$taxData, time_genus_da$FUN)
)[order(phylum, class, order, family), ] |> unique() |> printTable()

bind_rows(
  fullTaxa(BAC$taxData, exp_genus_da$BAC),
  fullTaxa(BAC$taxData, time_genus_da$BAC)
)[order(phylum, class, order, family), ] |> unique() |> printTable()
...
diffAbundanceHeatmapCombined <- function(data, ASVs, type, use_log1p = FALSE, genus = FALSE) {
  ...
  grid.grabExpr(draw(ht, heatmap_legend_side = "bottom"))
}

# Combined plots

exp_fun_genus_plot <- diffAbundanceHeatmapCombined(EXP_genus$FUN, exp_genus_da$FUN, "experiment", TRUE, TRUE)
exp_bac_genus_plot <- diffAbundanceHeatmapCombined(EXP_genus$BAC, exp_genus_da$BAC, "experiment", TRUE, TRUE)

agg_png(here(FIGURES_DIR, "genus_diff_EXP.png"), width = 20, height = 18, units = "cm")
wrap_elements(exp_fun_genus_plot) / wrap_elements(exp_bac_genus_plot) +
  plot_annotation(tag_levels = "A") + 
  plot_layout(heights = c(2, 4))
dev.off()


time_fun_genus_plot <- diffAbundanceHeatmapCombined(TIME_genus$FUN, time_genus_da$FUN, "timepoint", TRUE, TRUE)
time_bac_genus_plot <- diffAbundanceHeatmapCombined(TIME_genus$BAC, time_genus_da$BAC, "timepoint", TRUE, TRUE)

agg_png(here(FIGURES_DIR, "genus_diff_TIME.png"), width = 20, height = 25, units = "cm")
wrap_elements(time_fun_genus_plot) / wrap_elements(time_bac_genus_plot) +
  plot_annotation(tag_levels = "A") + 
