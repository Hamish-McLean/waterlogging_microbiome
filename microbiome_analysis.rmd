...
exp_cn_lm <- apply_to_subsets(EXP, genericAnova, update(E_DESIGN, log10_copy_number ~ .)) |> combineTables()
...
timepointAnova <- function(data, kingdom, source, design) {
  anova     <- aov(update(design, log10_copy_number ~ .), data$colData)
...
```{r}
exp_da$FUN |> combineTables() |> printTable()
```

#### EXP: BAC

```{r}
exp_da$BAC |> combineTables() |> printTable()
```
...
applyFunguilds(exp_da_FUN)
applyFunguilds(time_da_FUN)
