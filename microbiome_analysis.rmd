...
# adonis_analysis(BAC_exp$root$dds, e_design) %>% print()
...
differential_abundance <- function(dds, design, taxData) {
  design(dds) <- design
  dds <- DESeq(dds)
  res <- results(dds, contrast = c("treatment", "1", "0"))
  sig_asvs <- res[res$padj < 0.05 & !is.na(res$padj), ]
  data.table(
    ASV = rownames(sig_asvs),
    log2FoldChange = sig_asvs$log2FoldChange,
    padj = sig_asvs$padj,
    taxa = taxData[rownames(sig_asvs), ]$rank
  )
}

differential_abundance(FUN_exp$soil$dds, e_design, FUN$taxData)
differential_abundance(FUN_exp$root$dds, e_design, FUN$taxData)

differential_abundance(BAC_exp$soil$dds, e_design, BAC$taxData)
differential_abundance(BAC_exp$root$dds, e_design, BAC$taxData)
