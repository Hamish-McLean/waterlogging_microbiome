...
data_dir     <- file.path(here, "..", "Data") |> normalizePath() # Data directory
fig_dir      <- file.path(here, "figures", "qPCR")
...
load_qpcr_data <- function(
  file_path, plate, header_rows = 19, columns = data_columns
) {
  # file_path <- file.path(data_dir, filename)

  # Read the csv file and skip the header rows
  data <- suppressWarnings(fread(file = file_path, skip = header_rows, select = columns))
...
plate_1 <- load_qpcr_data(plate_files$plate_1, 1)
plate_2 <- load_qpcr_data(plate_files$plate_2, 2)
plate_3 <- load_qpcr_data(plate_files$plate_3, 3)
plate_4 <- load_qpcr_data(plate_files$plate_4, 4)
plate_5 <- load_qpcr_data(plate_files$plate_5, 5)
...
kbl(plate_1$data[Outlier != "ok"][order(Sample)], digits = 2, caption = "Plate 1 outliers")

plate_2$data <- flag_outliers(plate_2$data)
kbl(plate_2$data[Outlier != "ok"][order(Sample)], digits = 2, caption = "Plate 2 outliers")

plate_3$data <- flag_outliers(plate_3$data)
kbl(plate_3$data[Outlier != "ok"][order(Sample)], digits = 2, caption = "Plate 3 outliers")

plate_4$data <- flag_outliers(plate_4$data)
kbl(plate_4$data[Outlier != "ok"][order(Sample)], digits = 2, caption = "Plate 4 outliers")

plate_5$data <- flag_outliers(plate_5$data)
kbl(plate_5$data[Outlier != "ok"][order(Sample)], digits = 2, caption = "Plate 5 outliers")
...
plate_5$samples   <- plate_5$data[Content == "Sample"   & Outlier == "ok"]
plate_5$standards <- plate_5$data[Content == "Standard" & Outlier == "ok"]
...
plate_5$stats <- calculate_summary_statistics(plate_5$samples)

kbl(plate_1$stats, digits = 2, caption = "Plate 1 summary")
kbl(plate_2$stats, digits = 2, caption = "Plate 2 summary")
kbl(plate_3$stats, digits = 2, caption = "Plate 3 summary")
kbl(plate_4$stats, digits = 2, caption = "Plate 4 summary")
kbl(plate_5$stats, digits = 2, caption = "Plate 5 summary")
...
plate_5$samples <- merge(plate_5$samples, plate_5$stats, c("Sample", "Target"))
...
efficiency_adjustment(plate_5$samples)
...
plate_5$standards <- plate_5$standards[Concentration > 1e-5]
...
plate_5$standard_curve <- calculate_standard_curve(plate_5$standards)

kbl(plate_1$standard_curve, digits = 2, caption = "Plate 1 standard curve")
kbl(plate_2$standard_curve, digits = 2, caption = "Plate 2 standard curve")
kbl(plate_3$standard_curve, digits = 2, caption = "Plate 3 standard curve")
kbl(plate_4$standard_curve, digits = 2, caption = "Plate 4 standard curve")
kbl(plate_5$standard_curve, digits = 2, caption = "Plate 5 standard curve")
...
merge_efficiency(plate_5$standards, plate_5$standard_curve)
...
efficiency_adjustment(plate_5$standards)
...
plot_standard_curve(plate_5$standards, plate_5$standard_curve)
...
for (plate in list(plate_1, plate_2, plate_3, plate_4, plate_5)) {
...
plate_5$tcn_standard_curve <- calculate_standard_curve(
  plate_5$standards, adjust_cq = TRUE, use_tcn = TRUE
)
...
kbl(repeats(plate_1$data))
kbl(repeats(plate_2$data))
kbl(repeats(plate_3$data))
kbl(repeats(plate_4$data))
kbl(repeats(plate_5$data))
