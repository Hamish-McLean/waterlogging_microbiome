...
plate_4 <- load_qpcr_data("Data/2025-03-17 plate 4 soil 16S.csv",  4)
...
plate_4$samples <- merge(plate_4$samples, plate_4$stats, by = c("Sample", "Target"))
...
plate_4$standards <- plate_4$standards[Concentration > 1e-6]
...
repeats <- function(data) {
  repeats <- data[
    ...
    .SD[all(Outlier != "ok")],
    by = .(Sample, Target)
  ][, .(Plate, Type, Target, Sample, Outlier = "all outliers")]
  return(unique(repeats, by = "Sample")[order(Sample)])
}
...
plate_4$samples <- efficiency_adjustment(plate_4$samples)
